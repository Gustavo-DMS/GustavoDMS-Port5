...
def ContServico(con):
    try:
        cur = con.cursor()
        cur.execute("""SELECT COUNT(Servico_ID),servico.ID,Tipo_servico 
        FROM servico LEFT JOIN atendimento_servico
        ON servico.ID = atendimento_servico.Servico_ID
        INNER JOIN tipo_servico ON servico.Tipo_servico_ID = tipo_servico.ID
        GROUP BY (servico.ID) """)
    except Exception as e:
        print("Ocorreu o erro:",e) 
    else:
        resultado_cont = cur.fetchall()
        cur.close()
        return(resultado_cont)
...
def OverServico(con):
    try:
        ...
        resultado_over = cur.fetchall()
        cur.close()
        return(resultado_over)

def IncompatibilidadeSexo(con):
    try:
        cur = con.cursor()
        cur.execute("""SELECT servico.ID,servico.Descricao,pacientes.Nome,pacientes.sexo,frequencias.sexo
        FROM pacientes INNER JOIN atendimento ON pacientes.ID = atendimento.Paciente_ID
        INNER JOIN atendimento_servico ON atendimento.ID = atendimento_servico.Atendimento_ID
        INNER JOIN servico ON servico.ID = atendimento_servico.Servico_ID
        INNER JOIN frequencias ON servico.ID = frequencias.Servico_ID
